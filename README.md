...
## 아키텍쳐 및 구현 전략 
### 요구사항 해석
- 과제의 핵심을 단순히 알림을 발송하는 기능 구현이 아닌, 분산 시스템 환경에서 발생할 수 있는 동시성과 데이터 정합성 문제를 메시지 브로커 없이 애플리케이션 및 RDBMS 레벨에서 어떻게 제어하고 방어할 것인가에 대한 운영 및 설계 역량 평가로 해석했습니다. 이를 위해 락(Lock) 경합 최소화와 트랜잭션 범위 분리에 가장 큰 우선순위를 두었습니다.
### 1단계: 알림 최초 등록
